...
public class DatePickerDialogExample extends Activity {
    ...
    private DatePickerDialog.OnDateSetListener Datelistener = new DatePickerDialog.OnDateSetListener() {
        ...
        public void onDateSet(DatePicker v, int y, int m, int d) {
            ...
            if (y > t.get(Calendar.YEAR)) {
                flag = 0;
            } else if (y == t.get(Calendar.YEAR)) {
                if (m > t.get(Calendar.MONTH)) {
                    flag = 0;

                } else if (m == t.get(Calendar.MONTH)) {
                    if (d > t.get(Calendar.DAY_OF_MONTH))
                        flag = 0;
            ...
            if (flag == 0) {
                Toast.makeText(DatePickerDialogExample.this, "日期错误，重新设置", Toast.LENGTH_LONG
                ).show();
            }
            if (flag == 1) {
                year = y;
